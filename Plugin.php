...
class WriteAdmin_Plugin implements Typecho_Plugin_Interface
{
    /**
    ...
    public static function activate()
    {
        ...
        if (file_exists("var/Widget/Menu.php")) {
            ...
            copy("usr/plugins/WriteAdmin/var/Widget/Menu.php", "var/Widget/Menu.php");
        }
        if (file_exists("admin/header.php")) {
            ...
            if(version_compare(Typecho_Common::VERSION,'1.2.0') >=0){
                //挂载header.php
                copy("usr/plugins/WriteAdmin/admin/header.php", "admin/header.php");
            }else{
                //挂载header.php
                copy("usr/plugins/WriteAdmin/admin/header-old.php", "admin/header.php");
            }
    ...
    public static function config(Typecho_Widget_Helper_Form $form)
    {
        ...
        $url = $options->pluginUrl . '/WriteAdmin/static';
        ...
        $avatar = new Typecho_Widget_Helper_Form_Element_Text('avatar', NULL, NULL, _t('左边栏头像'), _t('输入头像链接，默认为QQ头像'));
...
--backgroundA: red!important;<br>
  ...
  --toggle-thuumb-bg: #24292f;<br>
  --toggle-track-border: #24292f;<br>
  --toggle-track-bg: #fff;<br>
}<br>黑夜<br>
[data-color-mode=dark] {<br>
  color-scheme: dark;<br>
  --background: #2c2c2c !important;<br>
  --backgroundA: #2c2c2c !important;<br>
  --WhiteDark: #000;<br>
  --WhiteDarkRe: #fff;<br>
  ...
  --toggle-track-bg: #2c2c2c;<br>
    ...
    public static function get_plugins_info(){
        $plugin_name = 'WriteAdmin'; //改成你的插件名
...
//        $activatedPlugins = json_decode(json_encode($activatedPlugins),true);
    ...
    public static function renderHeader($hed,$new)
    {
        ...
        $url = Helper::options()->pluginUrl . '/WriteAdmin/static/';
        if (!Typecho_Widget::widget('Widget_User')->hasLogin()) {
            $skin = Typecho_Widget::widget('Widget_Options')->plugin('WriteAdmin')->bgfengge;
            $diycss = Typecho_Widget::widget('Widget_Options')->plugin('WriteAdmin')->diycss;
            if ($skin == 'kongbai') {
                ...
                if ($skin == 'heike') {
                    ...
                    $bgUrl = Typecho_Widget::widget('Widget_Options')->plugin('WriteAdmin')->bgUrl;
                    $zidingyi = "";
                    if ($bgUrl) {
...
<script>$(document).ready(function(){if($("#modalinfo").length>0){$("#modalinfo").animatedModal();}});</script>';
            ...
            $url = Helper::options()->pluginUrl . '/WriteAdmin/';
            $skin = Typecho_Widget::widget('Widget_Options')->plugin('WriteAdmin')->bgfengge;
            $ft = '';
            if ($skin == 'heike') {
